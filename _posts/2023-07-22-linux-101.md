---
title: Linux 101
subtitle: Brutal gavage of linux commands. Also serves as my own cheat-sheet.
tags: linux
---
...
The filesystem works as a [tree](/graph-theory-101).

Unlike Windows where there can be multiple roots (think of `C:\`, `D:\` etc.), in Linux there is only one root and it is accessible via `/`. All other files, directories and partitions are located under the root. In particular, multiple drives are mounted under `/mnt`.
...
The **command** is the program or action to run. The **flags** are optional and modify the behavior of the command if needed. The **argument** is the input of the command.

For example, typing `df -Th /` displays the amount of available **disk space** for the root filesystem along with their type and human-readable numbers. We can break down this instruction as follows:
...
Below are basic commands to navigate the directory.
...
To interrupt a command, type. `Ctrl + C`
...
On the command-line, we can use Nano or Vi. The easiest way to add a line to use a file is to use `echo` as follows `echo "Hello World" > file1`.
...
Unline `ps` (process status), `top` (table of processes) auto-refreshes.
