...
Strategic Modeling is an abstraction exercise where software engineers and Domain experts gather and study the terminology used in a given Domain in order to uncover different Bounded Contexts, each Context having its own Ubiquitous Language (if that’s not abstract, I don’t know what is!).

By opposition, Tactical Modeling relies on a series of common coding “recipes” that are useful in implementing DDD. Typical recipes include Entities, Value Objects, Aggregates, Repositories etc.

It is worthwhile to note that simply using these coding patterns does not guarantee that the overall project will be properly modeled. In other words, Tactical Modeling patterns are (usually) necessary but not sufficient: if one chooses to model the software around DDD principles, it’s likely that one will end up using Entities, but it’s also possible to be using Entities without following the whole DDD approach.

# Terminology matters

In Philosophy, a good starting point when writing an essay is usually to look at the etymology of the very words used in the topic, which makes it possible to shed light on their full meaning and gain insight on the relationships between the concepts.
...
Sidenote: in Python, Value Objects would be either built-in immutable data types (bool, int, float, tuple, str and frozenset) or custom objects. In the latter case, the immutability would rely on coding conventions given that there is no such thing as private methods in Python. Also, the __eq__ magic method would have to be overwritten in order to test the equality of two objects by only looking at the value of their attributes.
