...
SHA-256 is an **asymmetric** hash function. Given the input "Hello World", it can quickly compute the corresponding hash. But when given a hash, it cannot recompute the original input. There is no such function as `sha256_reverse()`. We say that the hash function is **non-reversible** or **one-way**. The only way to determine the input for a given output is to try all possible inputs one by one, which is infeasible.
